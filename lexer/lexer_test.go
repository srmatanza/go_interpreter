...
func TestNextToken(t *testing.T) {
	...
	tests := []struct {
		...
		{token.LBRACKET, "["}, {token.INT, "0"}, {token.RBRACKET, "]"},
		...
		{token.IF, "if"}, {token.LPAREN, "("}, {token.TRUE, "true"}, {token.RPAREN, ")"}, {token.LBRACE, "{"},
		...
		{token.INT, "10"}, {token.LTE, "<="}, {token.INT, "10"}, {token.NEQ, "!="}, {token.INT, "9"}, {token.GTE, ">="}, {token.INT, "9"}, {token.SEMICOLON, ";"},
